...
func SetBackupFlagDefaults(flagSet *pflag.FlagSet) {
	...
	flagSet.Int(SINGLE_DATA_FILE_COPY_PREFETCH, 1, "Number of COPY commands to prefetch. May improve backup times when dataset contains many small tables")
...
func SetRestoreFlagDefaults(flagSet *pflag.FlagSet) {
	...
	flagSet.Int(SINGLE_DATA_FILE_COPY_PREFETCH, 1, "Number of COPY commands to prefetch. May improve restore times when dataset contains many small tables")
